...
回测的全部操作（建账户 / 开会话 / advance / close / 取报告）统一走 **HTTP 接口**，
不再提供命令行协议客户端。会话契约见 `design/18-session-backtest-engine.md`，
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_serve.add_argument(
        "--port", type=int, default=int(os.getenv("VORTEX_BACKTEST_PORT", "8766"))
    )
